...
export function ResourceCard({ resource, className = "", style }: ResourceCardProps) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.99 }}
      className={cn(
        "group block p-4 rounded-[1.25rem] border border-[var(--color-border)] bg-[var(--color-surface)]",
        "transition-all duration-300 hover:border-[var(--color-border-strong)] hover:shadow-xl",
        className
      )}
    >
      {thumbnail && (
        <div className="relative aspect-video rounded-xl overflow-hidden mb-4 bg-[var(--color-gray-100)] shadow-inner">
          <img
            src={thumbnail}
            alt={resource.title}
