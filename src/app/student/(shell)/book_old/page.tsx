    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div>
          ...
          <p className="text-text-dim text-base font-medium mt-1">Book professional counseling</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <select className="h-9 rounded bg-surface border border-border px-3 text-base font-medium text-text-muted outline-none focus:border-white/20 transition-colors ">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {counselors.map(counselor => (
          <motion.div key={counselor.id}>
            <Card padding="lg" className="bg-surface border-border group hover:border-white/20 transition-all flex flex-col items-center text-center">
              ...
              <Text variant="small" className="text-text-dim text-base font-medium mb-6">Psychologist · Lovely Professional University</Text>
              <div className="flex flex-wrap justify-center gap-2 mb-8">
                <span className="badge badge-outline text-base px-2 py-0.5 border-white/5 bg-white/5">General Support</span>
                <span className="badge badge-outline text-base px-2 py-0.5 border-white/5 bg-white/5">Anxiety</span>
              </div>

              <div className="flex items-center gap-2 mb-8">
                <div className="flex gap-0.5 text-warning">
                  {[1, 2, 3, 4, 5].map(i => <Icon key={i} icon="tabler:star-filled" className="text-base" />)}
                </div>
                {/* Use a deterministic value based on id length to avoid hydration mismatch */}
                <Text variant="small" className="text-text-dim text-base font-bold tabular-nums">4.9 · {50 + (counselor.id.length * 3) % 100} sessions</Text>
              </div>
              ...
              <div className="w-full pt-6 border-t border-white/5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-base font-medium text-success ">Available</span>
                  </div>
                  <Text variant="small" className="text-text-muted text-base font-medium ">Next: Today</Text>
                </div>
        ...
        {counselors.length === 0 && !isLoading && (
          <div className="col-span-full py-20 text-center text-text-dim border border-dashed border-border rounded-lg bg-white/[0.01]">
            <Icon icon="tabler:users-off" className="text-2xl mx-auto mb-4 opacity-10" />
            <p className="text-[1.0625rem] font-medium italic opacity-60">No counselors available in this view.</p>
          </div>
        )}
      </div>
      ...
      <Card padding="lg" className="bg-surface border-border">
        <div className="flex items-center justify-between mb-10 px-1">
          ...
          <span className="badge badge-primary bg-primary/10 text-primary border-primary/20 text-base">{existingBookings.length} Upcoming</span>
        </div>
        ...
        <div className="space-y-3">
          {existingBookings.map(booking => (
            <div key={booking.id} className="group flex flex-col md:flex-row items-center gap-6 p-4 rounded-lg bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
              <div className="flex flex-col items-center justify-center size-16 rounded bg-surface-raised border border-border shadow-sm shrink-0">
                <span className="text-base font-medium text-text-muted">{new Date(booking.slot_start).toLocaleDateString('en-IN', { weekday: 'short' })}</span>
                <span className="text-xl font-medium text-white leading-tight tabular-nums">{new Date(booking.slot_start).getDate()}</span>
                <span className="text-base font-medium text-text-dim ">{new Date(booking.slot_start).toLocaleDateString('en-IN', { month: 'short' })}</span>
              </div>
              
              <div className="flex-1 text-center md:text-left min-w-0">
                <Text weight="semibold" className="text-white text-[1.0625rem] mb-1">{booking.counselor?.name || "Counselor"}</Text>
                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-base font-medium text-text-dim tabular-nums">
                  <span className="flex items-center gap-1.5"><Icon icon="tabler:clock" className="text-primary text-base" /> {new Date(booking.slot_start).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>
                  <span className="flex items-center gap-1.5"><Icon icon={booking.type === 'online' ? "tabler:video" : "tabler:building"} className="text-secondary text-base" /> {booking.type}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button className="h-8 px-4 rounded bg-white text-black text-base font-medium hover:opacity-90 active:scale-95 transition-all">Join</button>
                  <button className="h-8 px-4 rounded bg-white/5 text-text-muted text-base font-medium border border-white/5 hover:text-white transition-all">Edit</button>
                </div>
              </div>
            </div>
          ))}
          {existingBookings.length === 0 && (
            <div className="p-16 text-center text-text-dim border border-dashed border-white/5 rounded-lg bg-white/[0.01]">
              ...
              <p className="text-[1.0625rem] font-medium italic opacity-60">No upcoming appointments found.</p>
            </div>
          )}
        </div>
      </Card>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
            ...
            >
              ...
              <div className="space-y-8">
                <div>
                  <Text variant="small" weight="medium" className="text-text-dim mb-4 block">Session Type</Text>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button 
                        ...
                        className={cn(
                          "flex-1 flex items-center justify-center gap-2 h-10 rounded border transition-all text-base font-medium ",
                ...
                <div>
                  <Text variant="small" weight="medium" className="text-text-dim mb-4 block">Available Slots</Text>
                  <div className="grid grid-cols-3 gap-2">
                    {counselorSlots.length > 0 ? (
                      counselorSlots.map(slot => (
                        ...
                        >
                          <span className="text-base font-bold">{new Date(slot.slot_start).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>
                          <span className="text-[8px] font-medium opacity-60 mt-1">{new Date(slot.slot_start).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}</span>
                        </button>
                      ))
                    ) : (
                      <div className="col-span-3 p-8 text-center text-text-dim border border-dashed border-white/5 rounded italic text-base opacity-60">No slots available</div>
                    )}
                  </div>
                </div>

                <div>
                  <Text variant="small" weight="medium" className="text-text-dim mb-4 block">Message (optional)</Text>
                  <textarea 
                    className="w-full min-h-[100px] bg-background border border-border rounded-md px-4 py-3 text-[1.0625rem] text-white placeholder:text-text-dim focus:border-white/20 transition-all outline-none resize-none leading-relaxed" 
                ...
                <div className="pt-8 border-t border-white/5">
                  <div className="flex items-start gap-3 p-4 rounded-md bg-white/[0.02] border border-white/5 mb-8">
                    ...
                    <Text variant="small" className="text-text-dim text-base leading-relaxed">
