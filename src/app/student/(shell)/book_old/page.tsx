    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-1">
        <div>
          ...
          <p className="text-text-dim text-xs font-medium tracking-[0.15em] mt-1">Book professional counseling</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <select className="h-9 rounded bg-surface border border-border px-3 text-xs font-medium text-text-muted outline-none focus:border-white/20 transition-colors ">
        ...
        {counselors.map(counselor => (
          <motion.div key={counselor.id}>
            <Card padding="lg" className="bg-surface border-border group hover:border-white/20 transition-all flex flex-col items-center text-center">
              ...
              <Text variant="small" className="text-text-dim text-[10px] font-medium mb-6">Psychologist · Jammu University</Text>
              ...
              <div className="w-full pt-6 border-t border-white/5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Text variant="small" className="text-text-muted text-[10px] font-medium ">Next: Today</Text>
                </div>
      ...
      <Card padding="lg" className="bg-surface border-border">
        <div className="flex items-center justify-between mb-10 px-1">
          ...
          <span className="badge badge-primary bg-primary/10 text-primary border-primary/20 tracking-[0.2em] text-[9px]">{existingBookings.length} Upcoming</span>
        </div>
          ...
          {existingBookings.map(booking => (
            <div key={booking.id} className="group flex flex-col md:flex-row items-center gap-6 p-4 rounded-lg bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
              <div className="flex flex-col items-center justify-center size-16 rounded bg-surface-raised border border-border shadow-sm shrink-0">
                <span className="text-[9px] font-medium text-text-muted">{new Date(booking.slot_start).toLocaleDateString('en-IN', { weekday: 'short' })}</span>
                <span className="text-xl font-medium text-white leading-tight tabular-nums">{new Date(booking.slot_start).getDate()}</span>
                <span className="text-[9px] font-medium text-text-dim ">{new Date(booking.slot_start).toLocaleDateString('en-IN', { month: 'short' })}</span>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button className="h-8 px-4 rounded bg-white text-black text-[10px] font-medium hover:opacity-90 active:scale-95 transition-all">Join</button>
                  <button className="h-8 px-4 rounded bg-white/5 text-text-muted text-[10px] font-medium border border-white/5 hover:text-white transition-all">Edit</button>
                </div>
              </div>
            </div>
          ))}
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button 
                        ...
                        className={cn(
                          "flex-1 flex items-center justify-center gap-2 h-10 rounded border transition-all text-xs font-medium ",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {counselorSlots.length > 0 ? (
                      ...
                      <div className="col-span-3 p-8 text-center text-text-dim border border-dashed border-white/5 rounded italic text-xs opacity-60">No slots available</div>
                    )}
                  </div>
                </div>
